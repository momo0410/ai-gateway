...
pub fn run() {
    ...
    let app = builder
        .setup(|app| {
            ...
            //
            // 为什么不能放进下面的 `if !is_screenshot_demo()` 里：待迁移的数据在
            // `~/.ai-gateway`（更名后那一代，1.0.0~1.0.1 写过）。若演示模式跳过迁移，
            // 本次运行就会在空的 `~/.wb-switch` 下建出一份**不完整**的账号库 ——
            // 而「已完成迁移」正是迁移的跳过条件，于是真实数据从此再也搬不过来
            // （实测踩到过：演示运行写出 2 个账号，把旧目录的 8 个账号永久挡在门外）。
            //
