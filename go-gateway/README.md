...
| 🔄 **多账号池** | 按积分到期日分层选号（先烧快过期额度，同档平均分摊），Top-5 候选 + 防惊群 |
...
| 📈 **Token 用量统计** | 每次成功请求的上游 usage 按模型/账号/日期聚合，`GET /usage?days=N` 查询，`usage.json` 持久化、重启保留 |
...
| `pool.credit_refresh_interval` | `15m` | 积分到期巡检周期（驱动「先烧快过期额度」的分层选号） |
| `pool.credit_refresh_enabled` | `true` | 积分到期巡检开关；`false` 时到期分层仅依赖签到与宿主同步 |
...
两级策略：**先按积分到期日分层，同档内再平均分摊**。

1. 过滤：禁用 / 冷却 / 熔断 / 在途占满账号不参与
2. **到期分层**：按各账号「最近到期积分」的到期日分组，只保留最早到期的一档
   - 目的：优先消耗快过期的额度，避免积分作废
   - **同一天到期的账号属于同一档**，档内平均使用（不因某号积分多就多打）
   - 到期日**未知**的账号排最后，仅在其它账号都不可用时才轮到
3. 档内挑选：按 `weight = idleWeight + successRate ×3` 取 Top-5 后加权随机
   ...
   - 档内**不含 credits 项**：同档意味着紧迫度相同，按积分加权会让高积分号长期吃掉流量
4. 防惊群：跳过 100ms 内刚被选中的账号；全冷却时从非禁用、非余额耗尽的软冷却/熔断账号中选最早到期者顶班

> **举例**：A 的积分 25 日到期、B 的 29 日到期 → 流量全给 A；A 的快过期额度用完后，
> 它的最近到期日跳到 29 日，与 B 同级，于是两者各占一半。

**全员无到期信息时**自动退回原三因子口径
（`credits 比例 ×10 + idleWeight + successRate ×3`），与引入分层前的行为逐字一致。

#### 到期数据从哪来

到期日取自 billing 接口 `get-user-resource` 的套餐字段
（`DeductionEndTime` / `ExpiredTime` / `CycleEndTime`，国服与国际版均返回），
只统计**仍有剩余**的套餐 —— 已用尽的套餐到期日再早也没有意义。
